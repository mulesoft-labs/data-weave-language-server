...
class InsertDocumentationAction(service: ValidationService) extends CodeActionProvider {
  override def handles(action: CodeActionParams): Boolean = {
    val toolingService: WeaveDocumentToolingService = service.documentService().open(action.getTextDocument.getUri)
